...
CREATE OR REPLACE VIEW risk_stage AS
      SELECT 
        ...
        Sept_bill,
CASE
    WHEN Sept_delay IN ('Balance Cleared', 'Minimum Due Cleared', 'Dormant') 
        THEN 'Stage 1 - Performing'

    WHEN Sept_delay IN ('1 Month Delay', '2 Months Delay') 
        THEN 'Stage 2 - Elevated Risk'

    WHEN Sept_delay IN (
        '3 Months Delay', '4 Months Delay', '5 Months Delay',
        '6 Months Delay', '7 Months Delay', '8 Months Delay', '9 Months Delay'
    ) 
        THEN 'Stage 3 - Non-Performing'
